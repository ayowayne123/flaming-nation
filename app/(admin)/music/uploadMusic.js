...
function Upload({ onClose, onUpload }) {
  ...
  useEffect(() => {
    ...
    const fetchPrograms = async () => {
      try {
        const response = await fetch("https://flaming.grantsforme.xyz/api/v1/programs");
        if (!response.ok) {
    ...
    const fetchCategories = async () => {
      try {
        const response = await fetch("https://flaming.grantsforme.xyz/api/v1/audio_categories");
        if (!response.ok) {
  ...
  const createAudio = async () => {
    try {
      const response = await fetch("https://flaming.grantsforme.xyz/api/v1/audios", {
        method: "POST",
        headers: {
      ...
      setDuration("");
      setArtist("");
      setCategory(4); // Reset to default category
